...
`.devcontainer/compose.local.gen.yml`; they are regenerated by `initialize.sh`.
Put manual runtime overrides in `.codegeist/.local.env`, local Compose overrides
in `.codegeist/compose.local.yml`, and devcontainer image extensions in
`.codegeist/Dockerfile` instead.

## Local Dockerfile Extensions

Consuming repositories can extend the devcontainer image without editing the
`.devcontainer/` submodule by adding `.codegeist/Dockerfile` next to the local
`.codegeist` env and Compose files. During `initializeCommand`, `initialize.sh`
writes `.devcontainer/Dockerfile.merged.gen` from the kit's
`.devcontainer/Dockerfile` and appends `.codegeist/Dockerfile` as a
project-local fragment.

The `.codegeist/Dockerfile` file is optional and is not generated by the kit. A
root `Dockerfile` remains available for application images and is not treated as
a devcontainer extension. When present for this devcontainer pattern, treat
`.codegeist/Dockerfile` as an extension fragment:
...
as `Dockerfile`; do not add a tracked root `Dockerfile` to the source checkout
for the kit base image.
...
`.env`, `Dockerfile.merged.gen`, and `compose.local.gen.yml`. They are ignored
by the kit's own `.gitignore` and should not be edited manually.
...
The `.devcontainer/.env`, `.devcontainer/Dockerfile.merged.gen`, and
`.devcontainer/compose.local.gen.yml` files are generated by the kit inside the
...
2. `initialize.sh` writes `.codegeist/compose.local.yml`.
...
6. `.devcontainer/docker-compose.yml` reads `.env` and
   `../.codegeist/.local.env` with `env_file`.
7. `devcontainer.json` includes `compose.local.gen.yml` and
   `../.codegeist/compose.local.yml`.
8. `.devcontainer/docker-compose.yml` owns the workspace and parent Git mounts.
9. `devcontainer.json` uses `${localEnv:USER}` for `remoteUser` and
   `containerUser`.
10. Container-side tools read normal environment variables from Compose.
...
- `.devcontainer/.env`, `.devcontainer/Dockerfile.merged.gen`, and
  `.devcontainer/compose.local.gen.yml` are regenerated by `initializeCommand`
...
.codegeist/
  ...
  Dockerfile            # optional project-local devcontainer extension fragment
...
- `docker-compose.yml` defines the workspace runtime and the root/worktree bind
  mounts.
- `Dockerfile` is the kit base image file; an optional `.codegeist/Dockerfile`
  can extend it through the generated `Dockerfile.merged.gen` file.
...
The generated `.codegeist` files intentionally live one directory above
`.devcontainer/`. This keeps machine-local state out of the vendored subtree so
many consuming projects can share the same `.devcontainer` kit without modifying
those subtree files, while keeping devcontainer-specific local files grouped
together.

The generated `.env`, `Dockerfile.merged.gen`, and `compose.local.gen.yml` files
intentionally live in `.devcontainer/` because they are kit-owned dynamic runtime
state. They are rewritten by `initialize.sh`; users should put manual overrides
in `.codegeist/.local.env`, `.codegeist/Dockerfile`, and
...
`.devcontainer/compose.local.gen.yml`, and `.codegeist/compose.local.yml`
...
- committed root `.env`, `.codegeist/.local.env`, or
  `.codegeist/compose.local.yml`
