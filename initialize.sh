...
dockerfile_example_path() {
  if [ -f "$script_dir/Dockerfile.example" ]; then
    printf '%s\n' "$script_dir/Dockerfile.example"
    return 0
  fi
...
ensure_codegeist_dockerfile() {
  local root_dir="$1"
  local target_file="$root_dir/.codegeist/Dockerfile"
  local example_file=""
  ...
  if ! example_file="$(dockerfile_example_path)"; then
    printf 'Dockerfile extension example is missing: %s\n' "$script_dir/Dockerfile.example" >&2
    return 1
  fi
...
#
# Do not edit manually. Root Compose overrides belong in ../.codegeist/compose.local.yml.
services:
  workspace:
    build:
      args:
...
main() {
  ...
  case "${1:-}" in
    "")
      ...
      ensure_codegeist_dockerfile "$root_dir"
