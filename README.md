...
Put manual overrides in root `.local.env`, root `Dockerfile`, and
...
`.devcontainer/` submodule by adding a root `Dockerfile` next to
`compose.local.yml`. During `initializeCommand`, `initialize.sh` writes
`.devcontainer/Dockerfile.merged.gen` from the kit's `.devcontainer/Dockerfile`
and appends the root `Dockerfile` as a project-local fragment.

The root `Dockerfile` is optional and is not generated by the kit because many
projects already use that filename for application images. When present for this
devcontainer pattern, treat it as an extension fragment:
...
Do not put `FROM` in the root fragment. A `FROM` instruction would start a new
...
6. `.devcontainer/docker-compose.yml` reads `.env` and `../.local.env` with
   `env_file`.
...
- `docker-compose.yml` defines the workspace runtime and the root/worktree bind
  mounts.
- `Dockerfile` is the kit base image file; an optional root `Dockerfile` can
...
in root `.local.env`, root `Dockerfile`, and `compose.local.yml` instead.
...
`.local.env` from `.devcontainer/.local.env.example` when missing, and links the
worktree `.local.env` back to the main root file. When the devcontainer starts
from an already selected checkout, `initializeCommand` writes that checkout's
