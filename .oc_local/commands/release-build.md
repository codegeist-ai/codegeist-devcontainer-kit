...
7. Record the pushed release commit with
   `git rev-parse release` and
   `git ls-remote --heads origin refs/heads/release`.
8. Fetch the release branch inside the `.devcontainer` submodule with
   `git -C .devcontainer fetch origin release`.
9. Check out the just-pushed release commit in the `.devcontainer` submodule with
   `git -C .devcontainer checkout origin/release`.
10. Verify `.devcontainer` points at the same release commit with
   `git -C .devcontainer rev-parse HEAD`,
   `git submodule status .devcontainer`, and
   `git -C .devcontainer log -1 --format=%s`.
11. Verify the parent repository shows the expected `.devcontainer` gitlink
    change with `git --no-pager status --short --branch`.
12. Report the release branch name, local release commit, remote release commit,
