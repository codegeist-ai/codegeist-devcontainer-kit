...
- The kit now supports consuming-repository root `Dockerfile` fragments for
  devcontainer image extensions without editing the `.devcontainer/` submodule.
  `initialize.sh` generates `.devcontainer/Dockerfile.merged.gen` from the kit
  Dockerfile and appends the root fragment when present; fragments must not
  contain `FROM`.
...
- When this source repository consumes its own runtime `.devcontainer/`
  submodule, the root `Dockerfile` is the kit source file, not a consumer
...
- Generated runtime files such as `.local.env`, `.devcontainer/.env`,
  ...
  `.oc_local/`, `compose.local.yml`, and `.worktrees/` should stay untracked
  unless a consuming repository explicitly owns an overlay.
...
- The suite covers initialization, Compose config resolution, branch worktree
  ...
  consuming-repo submodule workflow. It also covers root `Dockerfile` fragment
  merging, source-repo self-hosting with an identical root Dockerfile, and
