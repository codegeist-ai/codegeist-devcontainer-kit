...
Do not edit `.devcontainer/.env` or
...
`.env` and `compose.local.gen.yml`. They are ignored by the kit's own
...
The `.devcontainer/.env` and `.devcontainer/compose.local.gen.yml` files are
...
- write `.devcontainer/.env` and `.devcontainer/compose.local.gen.yml`
...
5. `.devcontainer/docker-compose.yml` reads `.env` and `../.local.env` with
   `env_file`.
...
- `.devcontainer/.env` and `.devcontainer/compose.local.gen.yml` are
...
The generated `.env` and `compose.local.gen.yml` files intentionally live in
...
`compose.local.yml` without nesting another worktree for the same branch.
