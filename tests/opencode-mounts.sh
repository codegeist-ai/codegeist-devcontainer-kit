...
#
# Why this exists:
# - docker-compose.yml bind-mounts host OpenCode directories into the
#   devcontainer so tools can reuse local state.
...
[[ "$default_config" != *"chrome-cdp-profile"* ]] || fail "Compose still contains the shared Chrome CDP profile mount"
[[ "$default_config" != *"CHROME_CDP_PROFILE_DIR"* ]] || fail "Compose still injects CHROME_CDP_PROFILE_DIR"
...
[[ "$compose_config" != *"chrome-cdp-profile"* ]] || fail "Compose override still contains the shared Chrome CDP profile mount"
