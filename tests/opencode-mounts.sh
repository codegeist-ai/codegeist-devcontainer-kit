...
cleanup_compose() {
  if [ -d "$fixture_dir" ]; then
    (cd "$fixture_dir" && env UID="$host_uid" docker compose -p "$compose_project" -f ".devcontainer/docker-compose.yml" down -v --remove-orphans >/dev/null 2>&1) || true
  fi
}
trap cleanup_compose EXIT

default_config="$(cd "$fixture_dir" && env -u OPENCODE_DIR_CONFIG -u OPENCODE_DIR_SHARE -u OPENCODE_DIR_STATE UID="$host_uid" docker compose \
...
compose_config="$(cd "$fixture_dir" && OPENCODE_DIR_CONFIG="$config_dir" OPENCODE_DIR_SHARE="$share_dir" OPENCODE_DIR_STATE="$state_dir" env UID="$host_uid" docker compose \
  -p "$compose_project" \
  -f ".devcontainer/docker-compose.yml" \
  config)"
