...
`.codegeist/compose.local.yml` or `.codegeist/Dockerfile`; they are visible root
extension files created from `.devcontainer/compose.local.yml.example` and
`.devcontainer/Dockerfile.example` when missing.
...
writes `.devcontainer/Dockerfile.merged.gen` from the release kit base at
`.devcontainer/Dockerfile` and appends root `.codegeist/Dockerfile` as a
project-local fragment.

The `.codegeist/Dockerfile` file is created from
`.devcontainer/Dockerfile.example` when missing and remains visible to Git. A root
`Dockerfile` remains available for application images and is not treated as a
devcontainer extension. Treat `.codegeist/Dockerfile` only as an extension
fragment:
...
as `Dockerfile` and ships `Dockerfile.example` as the template for root
...
- `.codegeist/compose.local.yml` remains visible to Git so repository overrides
...
.codegeist/
  ...
  compose.local.yml     # created when missing, visible to Git
  Dockerfile            # created when missing, visible extension fragment
...
- `compose.local.yml.example` is copied to `.codegeist/compose.local.yml` when
  missing; the result is visible to Git.
- `Dockerfile.example` is copied to `.codegeist/Dockerfile` when missing; the
...
machine-local and ignored; `.codegeist/compose.local.yml` and
